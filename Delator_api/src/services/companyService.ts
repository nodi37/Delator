...
import { saveNewSettings } from './companySettingsService';


const saveNewCompany = async (body: IDynamicObject) => {
    try {
        const settings = await saveNewSettings();
        const response = await new Company({...body, settingsId: settings!._id }).save();
